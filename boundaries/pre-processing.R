...
# December 2021, generalised resolution (20m)
...
# The default API call shown on each dataset page returns features for the whole county, therefore it's best to use some extra parameters to reduce the amount of data being returned and speed up the process.
# Local Authority (LA) boundaries can be obtained using the area code or name, e.g. E08000009 or "Trafford".
# For boundaries within an LA that don't contain the LA's area code or name you can define a rectangle (spatial envelope) around the LA and then use the st_intersection() function to obtain the features that intersect with the LA's boundary, or use a lookup.
...
la <- st_read("https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Local_Authority_Districts_December_2021_GB_BGC/FeatureServer/0/query?outFields=*&where=UPPER(lad21cd)%20like%20%27%25E08000009%25%27&f=geojson") %>% 
...
# Source: ONS Open Geography Portal
# URL: https://geoportal.statistics.gov.uk/datasets/ons::westminster-parliamentary-constituencies-december-2021-uk-bgc/about
# Licence: OGL v3.0
st_read(paste0("https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Westminster_Parliamentary_Constituencies_DEC_2021_UK_BGC/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson", api_geommetry_envelope)) %>% 
  st_intersection(la) %>% 
  select(area_code = PCON21CD, area_name = PCON21NM) %>% 
...
# Electoral wards -------------------------
# Source: ONS Open Geography Portal
# URL: https://geoportal.statistics.gov.uk/datasets/ons::wards-december-2021-gb-bgc/about
# Licence: OGL v3.0
wards <- st_read(paste0("https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Wards_December_2021_GB_BGC/FeatureServer/0/query?&outFields=*&where=1%3D1&f=geojson", api_geommetry_envelope)) %>% 
  st_intersection(la) %>%
  select(area_code = WD21CD, area_name = WD21NM) %>% 
...
# Parishes -------------------------
# Source: ONS Open Geography Portal
# URL: https://geoportal.statistics.gov.uk/datasets/ons::parishes-december-2021-ew-bgc/about
# Licence: OGL v3.0
st_read(paste0("https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Parishes_December_2021_EW_BGC/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson", api_geommetry_envelope)) %>% 
  st_intersection(la) %>%
  select(area_code = PAR21CD, area_name = PAR21NM) %>%
  # we need to filter further as some features returned are not applicable
